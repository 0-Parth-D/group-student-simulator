...
from app.knowledge.task_answer import ANSWER_STRATEGIES, compute_expected_answer
from app.knowledge.task_tagger import rule_classify
from app.knowledge.turn_classifier import SOCIAL_PATTERNS, _has_math_content, _heuristic_classify
...
def test_families_live_in_metadata_not_python():
    source = open("src/app/knowledge/task_tagger.py", encoding="utf-8").read()
...
def test_mistakes_reads_kg_cues_not_a_local_dict():
    import app.knowledge.mistakes as m
...
def test_turn_classifier_has_no_demo_roster_names():
    ...
    for name in ("jordan", "alex", "sam", "morgan"):
...
def test_morgan_persona_check_is_task_id_free():
    turn = {
        "profile_id": "morgan",
        "behavior_mode": "NORMAL_ERROR_PROFILE",
        "turn": 1,
