...
from app.prediction import merge_concepts
...
def is_math_press_turn(teacher_utt: str) -> bool:
    """Teacher is pressing for math reasoning — never stall on these."""
    cleaned = strip_addressing_noise(teacher_utt or "").strip().lower()
    if not cleaned:
        return False
    if "?" in cleaned:
        return True
...
def should_student_stall(
    ...
    """True only for high-precision vague turns with no concrete next action.

    Mis-tagged scaffolds (e.g. "okay, what numbers…") must NOT stall — that inject
    overrides transcript context and produces "what do you want me to do next?".
    """
    ...
    cleaned = strip_addressing_noise(teacher_utt or "")
    if is_math_press_turn(cleaned):
        return False
    if is_actionable_scaffold(cleaned):
        return False
    if not is_high_precision_vague(cleaned, current_label):  # type: ignore[arg-type]
        return False

    stripped = cleaned.strip().lower().rstrip("!.?")
    ...
    # Genuine empty continue / ack-only after the student has already spoken.
    return True
...
def prepare_student_user_message(
    ...
    Stall inject is gated on stall_active only — never on vague turn_mode alone —
    so a misclassified scaffold cannot force "what should I do next?".
    """
