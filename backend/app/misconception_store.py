...
from app.behavior_router import BehaviorTarget
...
MISCONCEPTION_INSTANCES = {
    ...
    "pr_table_missing_warrants": [
        "I made a table up to 50 texts and Plan B is cheaper. Like at 50 it's $15 and Plan A is $25. So... Plan B.",
        "Up to 50 texts Plan B looks better on my table — $15 vs $25 — but I can't say why it stays that way.",
        "I listed some numbers and Plan B was cheaper. I don't know how that connects to the equations.",
        "My table goes to 50. Plan B wins those rows. That's all I've got.",
    ],
        ...
        points.append(
            PointStruct(
                ...
                payload={
                    ...
                    "implied_plan": item.get("implied_plan", ""),
                    "discourse_role": item.get("discourse_role", ""),
...
def retrieve_error_instance(
    ...
    seed = (seed_claim or "").strip()
    query = (
        f"Student weak in {construct_id} holding claim: {seed} "
        f"responding to teacher: {teacher_utterance}"
        if seed
        else f"Student weak in {construct_id} responding to teacher: {teacher_utterance}"
    )
...
def _score_text_against_items(query: str, items: List[dict], k: int = 3) -> List[dict]:
    """Rank catalog rows by embedding similarity; keyword overlap if embedder fails."""
    if not items:
        return []
    q = (query or "").lower()
    if not q:
    ...
    try:
        ...
        for item in items:
            blob = " ".join(
                str(item.get(key) or "")
                for key in ("description", "prompt_cue", "example_wrong_reply", "id")
            ).lower()
...
def _qdrant_catalog_retrieve_ids(
    query_text: str,
    misc_ids: List[str],
    k: int = 3,
) -> Optional[List[dict]]:
    ids = [m for m in misc_ids if m]
    ...
    try:
        hits = _qdrant_vector_search(
            ...
            vector,
            query_filter=Filter(
                must=[FieldCondition(key="id", match=MatchAny(any=ids))]
            ),
            limit=max(k, len(ids)),
        )
    except Exception as exc:
        logger.warning("Qdrant fight catalog search failed: %s", exc)
        return None
    if not hits:
        return None
    results = []
