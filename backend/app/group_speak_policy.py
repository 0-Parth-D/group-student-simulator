...
def should_i_speak(
    ...
    *,
    ...
    if mode == "WRONG":
        if extraversion < 0.4:
            score += 0.05
            reasons.append("quiet misconception (low E)")
        else:
            score += 0.25
    ...
    if prior_student == pid:
        score -= 0.25 if constraints.mode == "critique" else 0.75
        reasons.append("spoke last round")

    score = round(max(0.0, min(1.0, score)), 2)
    cutoff = threshold
    if cutoff is None:
        cutoff = 0.45 if constraints.mode == "discuss" else 0.50 if constraints.mode == "critique" else 0.55
    ...
    *,
    ...
    for pid in group.profile_ids:
        ...
        if last_speaker and pid == last_speaker:
            score = round(max(0.0, decision.score - 0.2), 2)
            by_id[pid] = SpeakDecision(
                pid,
                score >= cutoff,
                score,
