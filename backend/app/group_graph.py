...
from app.group_context import eligible_speakers, trim_private_hist
from app.group_orchestrator import SpeakConstraints
from app.group_speak_policy import volunteer_select
from app.turn_logic import prepare_peer_user_message
...
def _generate_reply(state: PeerContinuationState) -> PeerContinuationState:
    ...
    decisions = state.get("speak_decisions") or []
    peer_reason = next(
        (
            d.get("reason")
            for d in decisions
            if d.get("profile_id") == selected and d.get("will_speak")
        ),
        None,
    )
    if not peer_reason and decisions:
        peer_reason = decisions[0].get("reason")
    reply = {
        "speaker_id": selected,
        "content": reply_text,
        ...
        "speak_reason": peer_reason or "peer volunteer",
    }
