from app.group_orchestrator import (
...
def test_direct_address_selects_only_named_student():
    constraints = parse_speak_constraints(
        "Maya, what do you think?", PROFILE_IDS, DISPLAY_NAMES
    )
    assert constraints.must_speak == ["maya"]
    assert constraints.mode == "direct"


def test_discuss_excludes_student_told_to_watch():
    constraints = parse_speak_constraints(
        "Alex and Maya discuss; Jordan please watch",
        PROFILE_IDS,
        DISPLAY_NAMES,
    )
    assert constraints.may_speak == ["alex", "maya"]
    assert constraints.must_not_speak == ["jordan"]


def test_topic_only_name_is_not_directly_addressed():
    constraints = parse_speak_constraints(
        "Maya, explain why Jordan's method might be wrong",
        PROFILE_IDS,
        DISPLAY_NAMES,
    )
    assert constraints.must_speak == ["maya"]
...
def test_legacy_mode_keeps_first_roster_default():
    ...
    assert resolution.speakers == ["alex"]
...
def test_observer_never_replies(monkeypatch):
    from app.group_orchestrator import parse_speak_constraints
    ...
    monkeypatch.setattr(
        "app.group_constraint_llm.resolve_constraint_parser",
        lambda name=None: parse_speak_constraints,
    )
    _, replies, vague_warning, active, turn_mode, multi, stop_reason = store.respond(
        group, "Alex and Maya discuss; Jordan please watch"
    )

    assert [reply["speaker_id"] for reply in replies] == ["alex", "maya"]
    assert group.last_observers == ["jordan"]
    assert vague_warning is False
    assert all(r["speaker_id"] != "jordan" for r in multi)


def test_respond_emits_stream_events(monkeypatch):
    """on_event should fire speak_plan / thinking / reply before respond returns."""
    from app.group_orchestrator import parse_speak_constraints
    ...
    monkeypatch.setattr("app.config.GROUP_GAME_FLOW", False)
    monkeypatch.setattr("app.config.GROUP_MULTI_ROUND", False)
    ...
    assert types.index("speak_plan") < types.index("reply")
    reply_events = [e for e in events if e.get("type") == "reply"]
    assert reply_events[0]["reply"]["speaker_id"] == "maya"
    assert reply_events[0]["kind"] == "primary"
    from app.group_orchestrator import parse_speak_constraints
    ...
    _, replies, *_rest = store.respond(group, "Maya, what do you think?")

    assert group.started is True
    assert group.turn == 1
    assert len(replies) >= 1
    assert replies[0]["speaker_id"] == "maya"
    teacher_entries = [
        t for t in group.transcript if t.get("speaker_id") == "teacher"
    ]
    assert len(teacher_entries) == 1
    assert teacher_entries[0]["content"] == "Maya, what do you think?"
