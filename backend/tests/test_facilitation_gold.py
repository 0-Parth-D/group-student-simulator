...
def test_gold_file_loads_and_has_personality_brief():
    ...
    assert gold["roster"]["profile_ids"] == ["alex", "maya", "jordan"]
    assert set(gold["personality_brief"]) == {"alex", "maya", "jordan"}
...
def test_topic_only_keeps_jordan_out_of_must_speak():
    summary = score_regex_baseline()
    row = next(
        r for r in summary["rows"] if r["id"] == "topic_jordan_method_maya_explains"
    )
