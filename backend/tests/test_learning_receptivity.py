...
def _group(mastery: dict | None = None) -> GroupSession:
    ...
    slots = {}
    for pid, name in (("alex", "Alex"), ("maya", "Maya"), ("jordan", "Jordan")):
...
def test_receptivity_bands_are_narrow_and_ordered():
    ...
    # Jordan (high C/O, low N) should be at least as receptive as Riley.
    assert PROFILE_RECEPTIVITY["jordan"] >= PROFILE_RECEPTIVITY["riley"]
    assert PROFILE_RECEPTIVITY["alex"] >= PROFILE_RECEPTIVITY["riley"]
