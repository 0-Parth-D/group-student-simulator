...
from app.student.behavior_router import error_applies_at_mastery
from app.knowledge.kg_config import MASTERY_LABELS, get_construct_def

from app.student.models import Student

from app.student.personality import (
...
def _format_playbook(
    ...
    shown = 0
    for p in error_playbook.get("patterns", [])[:5]:
...
def student_prompt(
    ...
    if is_math and student.construct_mastery and top_p_concepts:
        kg_summary = "\n".join(
            f"  - {(get_construct_def(c) or {}).get('label', c)}: "
            f"{MASTERY_LABELS.get(student.construct_mastery.get(c, 0), 'Unknown')}"
    ...
    if is_math and behavior_profile:
        cognitive_block = build_cognitive_state_block(
            student, behavior_profile, error_playbook, turn_mode,
            stall_active=stall_active,
        )
        personality_block = build_personality_expression_block(
            student, behavior_profile, turn_mode, stall_active=stall_active,
        )
    ...
    if is_math:
        if cognitive_block:
            lines.extend([cognitive_block, ""])
        if personality_block:
            lines.extend([personality_block, ""])
        ...
        lines.extend(
            [
                ...
                *_format_construct_block(student, behavior_profile),
                *_format_lp_mastery_block(behavior_profile),
                *_format_pisa_block(student, behavior_profile),
                *_format_behavior_profile_block(behavior_profile, scaffold_boost),
                "[KG-Based Cognitive Profile — active for this turn]",
                "Your mastery of relevant concepts for this step:",
                kg_summary,
                "",
                "[KG Realistic Rules]",
                "- When a Bad concept applies to THIS step in the problem, show a realistic process error: "
                "arithmetic slip, wrong operation, incomplete step, or confusion — not a random wrong fact.",
                "- When a concept is Good, use it correctly for that step.",
                "- Unknown/Bad in problem context: express confusion or a partial attempt. "
                "It is OK to get a small sub-step right and fail the next one.",
                "- Do NOT produce a perfect full solution.",
                "- On active math turns (not stall), do NOT ask empty process questions "
                "like 'what do you want me to do?' — attempt the step or ask about content.",
                "",
                *_format_playbook(
                    error_playbook, behavior_profile, student, stall_active=stall_active
                ),
                "[Behavior When You Don't Know]",
                "A concept is weak for you if it is labeled Bad or Unknown above.",
                "When the teacher asks about such a concept in the problem:",
                unknown_behavior_rule(student, turn_mode, behavior_profile),
                "",
                "[Predicted Behavior for this Task]",
                predicted_behavior,
                mistake_follow,
                "",
                *anchor_block,
